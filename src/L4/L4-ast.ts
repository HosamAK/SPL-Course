...
const parseBoundExp = (params: Sexp[]): Result<BoundExp> =>
    params.length === 0 ? makeFailure("No arguments") : 
    params.length > 1 ? makeFailure("Too many arguments") :
    parsegoodBoundExp(params[0]);

const parsegoodBoundExp = (v: Sexp): Result<BoundExp> =>
    !isIdentifier(v) ? makeFailure("Var must be an identifier") :
    mapv(parseL4CExp(v), () => makeBoundExp(makeVarRef(v)));
    
const parseTimeExp = (params: Sexp[]): Result<TimeExp> => 
    params.length === 0 ? makeFailure("No arguments") :
    params.length > 1 ? makeFailure("To many arguments") :
    parsegoodTimeExp(params[0]);

const parsegoodTimeExp = (exp: Sexp): Result<TimeExp> =>{
    const fin = parseL4CExp(exp);
    if(!isOk(fin))
        return makeFailure("exp must be identifier");
    
    return makeOk(makeTimeExp(fin.value));
}
